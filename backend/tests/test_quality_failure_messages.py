from app.agents.pipeline_messages import build_hitl_compile_failure_user_message, build_quality_failure_user_message
...
def test_quality_failure_points_to_interactive_regeneration():
    ...
    assert "retrieved scripture citations" in message
    assert "could not verify enough support" not in message
...
def test_hitl_compile_grounding_failure_mentions_selected_scriptures_not_starting_interactive_again():
    message = build_hitl_compile_failure_user_message(
        {
            "scores": {"citation_grounding": 2, "faithfulness": 4},
            "failedDimensions": ["citation_grounding"],
        },
